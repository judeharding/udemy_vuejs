...
export default {
  setup() {
    const availableFunds = 100; // constant value never changes
    const currentExpenses = ref(0);
    const enteredExpense = ref(0);

    const remainingFunds = computed(function() {
      return availableFunds - currentExpenses.value; // available funds are not .value
      // because they aren't computed
    });
